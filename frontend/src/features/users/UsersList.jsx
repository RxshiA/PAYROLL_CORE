...
function UsersList() {
    ...
    const searchedQuotation = user.filter((user) =>
    ...
    const generatepdf = async () => {
        await axios
            .post(`http://localhost:3500/users/createpdf`, user)
            .then((response) => {
                console.log(response);
                axios
                    .get('http://localhost:3500/users/fetchpdf', {
                        responseType: 'blob'
                    })
                    .then((res) => {
                        console.log(res);
        ...
        Swal.fire({
                icon: 'success',
             title: 'User Report Genarated successfully!'
          });
         navigate('/dash/users');
    };
    ...
    const handleDelete = (deleteID) => {
        ...
        axios.delete(`http://localhost:3500/users/${deleteID}`)
        .then(() => {     
                                    ...
                                    <Table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                        <TableHead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                            <TableRow>
                                                ...
                                                <TableCell align="right" className='px-6 py-3'>Roles</TableCell>
                                                <TableCell align="right" className='px-6 py-3'>Joined Date</TableCell>
                                            </TableRow>
                                        </TableHead>
                                        <TableBody>
                                        {user.length > 0 &&
                                            searchedQuotation.map((user) => (
                                            <tr key={user._id}>
